...
u(x,t) = \frac{1}{2 \pi D}
\int^\infty_{-\infty} \hat{u}_0 (k) e^{\imath kx} e^{-k^2t} dk
\end{equation}
...
We can test this solution by verifying that Equation \ref{eq:gensolution} yields $u(x,0)$ at $t=0$:
\begin{equation}
  \label{eq:initialcon}
...
\end{equation}
and comparing with Equation \ref{eq:initialcon}, we recover the arbitrary initial condition $u(x,0)$. Thus Equation \ref{eq:gensolution} is the general solution to Equation \ref{eq:heat}.

To move from this general solution to the overall solution, we first find a fundamental solution for an initial condition of a point source of $u$. We use this fundamental solution for a point source to build the solution that applies to any initial condition. A point source is defined as a delta function: \begin{math} u(x,0) = \delta (x) \end{math} at \begin{math} t=0 \end{math}. This is a convenient initial condition because its Fourier transform is \begin{math} \hat{u}_0 (k) = 1 \end{math}.

We plug this initial condition into Equation \ref{eq:gensolution}:
\begin{equation}
  \label{eq:refinitial}
u(x,t) = \frac{1}{2 \pi D} \int^\infty_{-\infty} e^{\imath kx} e^{-k^2t} dk,
\end{equation}
...
\end{equation}
where the first grouping will be \begin{math} u \end{math} and the second grouping will be \begin{math} dv \end{math}, such that:
\begin{eqnarray}
...
\end{equation}
The first term inside the square brackets goes to zero when evaluated from \begin{math} -\infty \end{math} to \begin{math} \infty \end{math}, and we are left with:
\begin{equation}
  \label{eq:partialx}
...
\begin{eqnarray}
\int^\infty_{-\infty} u(x,t) dx & = & \int^\infty_{-\infty} u(x,0) dx  \nonumber \\
& = & \int^\infty_{-\infty} \delta (x) dx \nonumber \\
& = & 1 \nonumber
\end{eqnarray}
We apply this conservation criterion to solve for $c$:
\begin{eqnarray}
\int^\infty_{-\infty} c e^{-x^2/4tD} dx = 1 \\
c = \frac{1}{\int^\infty_{-\infty} e^{-x^2/4tD} dx} \label{eq:c}
\end{eqnarray}
The denomenator is the integral of a Gaussian, which in general is given by:
\begin{eqnarray}
\int^\infty_{-\infty} e^{-ax^2} dx = \sqrt{\frac{\pi}{a}} \nonumber
\end{eqnarray}
In Equation \ref{eq:c}, \begin{math} a = \frac{1}{4tD} \end{math}, and thus we can solve for $c$ as
\begin{equation}
...
To build towards an overall solution, let us now consider the possibility that our initial condition delta function is instead located a different point \begin{math} x = s \end{math}, such that:
\begin{eqnarray}
u(x,0) = \delta (x-s) \quad \mbox{at} \quad t=0. \nonumber
\end{eqnarray}
Then the argument of the exponential in our solution shifts by $s$, such that:
\begin{eqnarray}
e^{-x^2/4tD} \quad \mbox{becomes} \quad e^{-(x-s)^2/4tD}. \nonumber
\end{eqnarray}
Because of linearity, any initial condition \begin{math} u(x,0) \end{math} can be written as the combination of point sources at locations $s$:
\begin{eqnarray}
u(x,0) = \int_{all \; s} \delta (x-s) u(s,0) ds \nonumber
\end{eqnarray}
And finally, the solution for an initial condition extending over all points in $x$ can be written as an integral of the responses to delta functions at many locations $\delta(x-s)$:
\begin{equation}
  \label{eq:difsol}
u(x,t) = \frac{1}{\sqrt{4 \pi tD}} \int^{\infty}_{-\infty} u(x,0)
e^{-(x-s)^2/4tD} ds
\end{equation}

Equation \ref{eq:difsol} gives the solution to the diffusion equation, which characterizes the diffusion of water isotopes in the firn layer. Because ice sheets also experience thinning that affects the water isotope profile, we multiply this equation by a thinning function $F(t)$ to account for this process:
\begin{equation}
  \label{eq:isoprofile}
u(x,t) = \frac{1}{\sqrt{4 \pi tD}} F(t) \int^{\infty}_{-\infty} u(x,0)
e^{-(x-s)^2/4tD} ds
\end{equation}
...
The same solution can be derived through a statistical framework, based on describing diffusion as a discrete random walk, as described in \citet{Lasaga2014}. A particle starts at a depth of \begin{math} z = 0 \end{math} and at each time step can move a vertical distance \begin{math} L \end{math} either upward with probability \begin{math} p \end{math} or downward with probability \begin{math} q = 1-p \end{math}. We would like to know what the probability is that after \begin{math} N \end{math} steps the particle is at some position \begin{math} z = mL \end{math}, where \begin{math} -N \leq m \leq N \end{math}.

Let us define the number of steps the particle takes upward, \begin{math} n_U \end{math}, and the number of steps the particle takes downward, \begin{math} n_D \end{math}. With these definitions we can write:
\begin{equation}
N = n_U + n_D \\
m = n_U - n_D
\end{equation}
...
We can write the probability, \begin{math} P_N(m) \end{math} of arriving at position \begin{math} z = mL \end{math} after N steps as the product of the probability of taking a particular sequence of steps to that position, \begin{math} p^{n_U} q^{n_D} \end{math}, times the number of different sequences of steps that will end at that position (because multiple sequences of steps will end at the same $z$ position, i.e. UUUD, UUDU, UDUU, and DUUU all end at \begin{math} m=2 \end{math}):
\begin{equation}
  \label{eq:prob1}
...
We can now make a couple of assumptions to simplify Equation \ref{eq:prob2}. First, we assume that \begin{math} p=q= \frac{1}{2} \end{math}, that the particle is equally likely to step upward or downward. Second, we can assume that the particle has taken many steps (\begin{math} N \gg 1 \end{math}) and that the number of steps taken is much greater than the distance from the starting point (\begin{math} N \gg m \end{math}). With these assumptions, we can use Stirling's Approximation:
\begin{equation}
...
[\frac{1}{2}(N+m)] \nonumber \\
& = & - \frac{1}{2} \ln[\frac{1}{2}(N-m)]- \frac{1}{2} \ln(2 \pi) + N \ln \frac{1}{2} \label{eq:prob2b}
\end{eqnarray}
...
This equation describes only the discrete probability of the particle taking steps of size \begin{math} L \end{math}. If we want an expression for the continuous probability \begin{math} W(z,t) \end{math} that the particle will be at any point \begin{math} z \end{math} at any time \begin{math} t \end{math}, we must generalize \begin{math} m \end{math} to \begin{math} z \end{math} and \begin{math} N \end{math} to \begin{math} t \end{math}. We define the relations:
\begin{equation}
...
Defining the diffusion length \begin{math} \sigma \equiv \sqrt{2Dt} \end{math}, pulling the constants outside the integral, and replacing \begin{math} c \end{math} with \begin{math} \delta \end{math}, \begin{math} z' \end{math} with \begin{math} z \end{math}, and \begin{math} z \end{math} with \begin{math} u \end{math}, we have again derived the solution for the diffused isotope profile:
\begin{equation}
...
The diffusion length \begin{math} \sigma \end{math}, the average vertical distance moved by a water molecule, can also be derived in a statistical manner, as in \citet{Lasaga2014}. We start with \begin{math} W(Z,\tau) \end{math}, the probability that a molecule at position \begin{math} z \end{math} at time \begin{math} t \end{math} will diffuse to position \begin{math} z + Z \end{math} at time \begin{math} t + \tau \end{math}. If the concentration profile at time \begin{math} t \end{math} is known, then the concentration after \begin{math} \tau \end{math} amount of time can be written as:
\begin{eqnarray}
...
\end{eqnarray}
where \begin{math} \langle \quad \rangle \end{math} represents a statistical average. With these definitions we can now write Equation \ref{eq:concentration2} as the basic diffusion equation:
\begin{eqnarray}
  \label{eq:heatcomp}
...
This equation is equivalent to the definition of diffusion length \begin{math} \sigma \end{math}, which is used to describe the ``extent" of diffusion. Equation \ref{eq:difflen} gives insight into the statistical meaning of the diffusion length as the root mean square of the vertical displacement.
...
The definition of transforming a Gaussian from the depth domain to the frequency domain is simple:
\begin{equation}
